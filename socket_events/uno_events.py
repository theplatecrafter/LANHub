...
rooms:        dict[str, dict] = {}   # room_id → room dict
...
def _emit_room(room_id: str):
    ...
    base = {
        ...
        'can_start':  _total_players(room) >= room['min_players'] and room['status'] == 'waiting',
...
def _broadcast_game(room_id: str):
    ...
    game = room['game']
    for p in room['players']:
        snap = game.snapshot(viewer_sid=p['sid'], viewer_is_spectator=False)
        socketio.emit('uno_game_state', snap, to=p['sid'])
    for s in room['spectators']:
        snap = game.snapshot(viewer_sid=s['sid'], viewer_is_spectator=True)
        socketio.emit('uno_game_state', snap, to=s['sid'])
...
def _handle_game_result(room_id: str, event: str, extra: dict = None):
    ...
    if event == 'player_won':
        ...
        won_sid  = next((p['sid'] for p in room['players'] if p['username'] == username), None)
        if won_sid:
            ...
            if won_sid in uno_sessions:
                uno_sessions[won_sid]['room_id'] = room_id  # still in room, just spectator
...
def _do_bot_turn(room_id: str, game: UnoGame):
    ...
    if game.waiting_for_color:
        color  = bot_choose_color(cur['hand'])
        result = game.choose_color(cur_idx, color)
    else:
        top    = game.top_card()
        ci     = bot_choose_card(cur['hand'], top, cur.get('difficulty', 'medium'),
                                  game.pending_draw)
        if ci is None:
            result = game.draw_action(cur_idx)
        else:
            card   = cur['hand'][ci]
            cc     = bot_choose_color(cur['hand']) if card.color == 'wild' else None
    ...
    ev = result.get('event')
    if ev in ('game_over', 'player_won'):
        _handle_game_result(room_id, ev, result)
    if ev != 'game_over' and game.status == 'playing':
...
def _cleanup_uno(sid: str):
    ...
    if room['status'] == 'waiting':
        ...
        # If creator left, transfer or close
        ...
        if game:
            ...
            if p_idx >= 0:
                ...
                app_log.info(f"[uno] {pdata['username']} replaced by bot in room {room_id}")
                # Notify room
                socketio.emit('uno_player_left', {'username': pdata['username']},
                              room=_rn(room_id))
                _broadcast_game(room_id)
                # Trigger bot turn if it's now that player's turn
...
def handle_add_bot(data):
    ...
    if diff not in ('easy', 'medium', 'hard'):
        diff = 'medium'
    used_names = {b['username'] for b in room['bots']}
    candidates = [n for n in _BOT_NAMES if f'Bot {n}' not in used_names]
    base_name  = f'Bot {random.choice(candidates) if candidates else uuid.uuid4().hex[:4]}'
    room['bots'].append({
...
def handle_remove_bot(data):
    ...
    room['bots'] = [b for b in room['bots'] if b['id'] != bot_id]
    _emit_room(room_id)
...
def handle_start_game(_=None):
    ...
    players_data = []
    for p in room['players']:
        players_data.append({'sid': p['sid'], 'id': p['sid'],
                              'username': p['username'], 'is_bot': False, 'difficulty': None})
    for b in room['bots']:
        players_data.append({'sid': None, 'id': b['id'],
                              'username': b['username'], 'is_bot': True,
                              'difficulty': b['difficulty']})

    room['game']   = UnoGame(players_data)
    room['status'] = 'playing'
    ...
    game = room['game']
    if game.current_player().get('is_bot'):
        _schedule_bot(room_id)

    app_log.info(f"[uno] Room {room_id} game started with {_total_players(room)} players")
...
def handle_play_card(data):
    ...
    result = game.play_card(player_idx, int(card_idx), chosen_color)
    if not result.get('ok'):
