...
def save_channel_message(channel_id: int, username: str, ip: str,
                         message: str, reply_to_id: int | None = None) -> dict:
    now  = time.time()
    conn = get_db()
    c    = conn.cursor()
    c.execute("""
        INSERT INTO channel_messages
            (channel_id, username, ip, message, timestamp, reply_to_id)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (channel_id, username, ip, message, now, reply_to_id))
    conn.commit()
    mid = c.lastrowid
...
def get_channel_messages(channel_id: int, limit: int) -> list[dict]:
    ...
    c    = conn.cursor()
    c.execute("""
        SELECT m.id, m.username, m.message, m.timestamp, m.edited,
    ...
    """, (channel_id, limit))
    rows = [dict(zip(
        ["id","username","message","timestamp","edited",
